...
elseif($status == 0) {
	...
	elseif(strpos($text, "/nidi") === 0 ) {
		...
		while ($row = mysqli_fetch_assoc($result)) {
			...
			$row2['lat'] != NULL ? array_push($latN, $row2['lat']) : array_push($latN,0);
			$row2['lng'] != NULL ? array_push($lngN, $row2['lng']) : array_push($lngN,0);
		}
		...
		while ($row = mysqli_fetch_assoc($result)) {
			...
			$result2 = mysqli_query($conn,$query2);
			$row2 = mysqli_fetch_assoc($result2);
			$row2['lat'] != NULL ? array_push($latS, $row2['lat']) : array_push($latS,0);
			$row2['lng'] != NULL ? array_push($lngS, $row2['lng']) : array_push($lngS,0);
		}
